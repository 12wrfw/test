...
do
    ...
    HttpService = Success and Service or nil;
end;
...
function Library:CreatePopup(Config)
    Config = Config or {};
    
    local Title = Config.Title or "Update Log";
    local WelcomeText = Config.WelcomeText or "Hello sir, Welcome to Exiled! Have a nice day >:3";
    local UpdateLogTitle = Config.UpdateLogTitle;
    if not UpdateLogTitle then
        local Success, DateStr = pcall(function() return os.date("%d/%m/%Y") end);
        UpdateLogTitle = "Update Log - " .. (Success and DateStr or "Unknown");
    end;
    local UpdateLogItems = Config.UpdateLogItems or {};
    local ShowDontShowAgain = Config.ShowDontShowAgain ~= false;
    local DontShowAgainKey = Config.DontShowAgainKey or "PopupDontShowAgain";
    local OnClose = Config.OnClose;
    local Width = Config.Width or 400;
    
    local function GetUpdateLogHash(Items)
        local Combined = table.concat(Items, "|");
        local Hash = 0;
        for i = 1, #Combined do
            Hash = ((Hash * 32) - Hash) + string.byte(Combined, i);
            Hash = Hash % 2147483647;
        end;
        return tostring(math.abs(Hash));
    end;
    
    local CurrentHash = GetUpdateLogHash(UpdateLogItems);
    local PopupDataFile = DontShowAgainKey .. "_data.json";
    
    local function LoadPopupData()
        local Success, Data = pcall(function()
            if writefile and readfile and isfile then
                if isfile(PopupDataFile) then
                    local FileContent = readfile(PopupDataFile);
                    if FileContent then
                        local HttpSuccess, Decoded = pcall(function()
            ...
            if Library.SaveManager then
                local SavedData = Library.SaveManager:GetData();
                return SavedData and SavedData[DontShowAgainKey .. "_data"];
            end;
            return nil;
        end);
    ...
    local function SavePopupData(Data)
        pcall(function()
            if writefile then
                local HttpSuccess, Encoded = pcall(function()
                    return HttpService:JSONEncode(Data);
                end);
                if HttpSuccess and Encoded then
                    writefile(PopupDataFile, Encoded);
                    return;
                end;
            end;
            if Library.SaveManager then
                local SavedData = Library.SaveManager:GetData() or {};
                SavedData[DontShowAgainKey .. "_data"] = Data;
                Library.SaveManager:SetData(SavedData);
                Library:AttemptSave();
            end;
        end);
    end;
    
    if ShowDontShowAgain then
        local PopupData = LoadPopupData();
        if PopupData.DontShow and PopupData.Hash == CurrentHash then
            return nil;
        end;
    end;
    ...
    local PopupOuter = Library:Create('Frame', {
        ...
        Size = UDim2.new(0, Width, 0, 200);
        ZIndex = 150;
        Parent = ScreenGui;
    });
    
    Library:MakeDraggable(PopupOuter, 25);
    ...
    local ContentContainer = Library:Create('ScrollingFrame', {
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
    ...
    Library:AddToRegistry(ContentContainer, {
        ScrollBarImageColor3 = 'AccentColor';
    });
    ...
    local MainBoxOuter = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Library.OutlineColor;
    ...
    Library:AddToRegistry(MainBoxOuter, {
        BackgroundColor3 = 'BackgroundColor';
        BorderColor3 = 'OutlineColor';
    });
    
    local MainBoxInner = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Color3.new(0, 0, 0);
        BorderMode = Enum.BorderMode.Inset;
        Position = UDim2.new(0, 0, 0, 0);
    ...
    Library:AddToRegistry(MainBoxInner, {
        BackgroundColor3 = 'BackgroundColor';
    });
    
    local MainHighlight = Library:Create('Frame', {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Size = UDim2.new(1, 0, 0, 1);
        ZIndex = 156;
    ...
    Library:AddToRegistry(MainHighlight, {
        BackgroundColor3 = 'AccentColor';
    });
    
    local MainSectionLabel = Library:CreateLabel2({
        Size = UDim2.new(1, 0, 0, 18);
    ...
    wait();
    local WelcomeTextHeight = select(2, Library:GetTextBounds(WelcomeText, Enum.Font.Code, 14, Vector2.new(Width - 48, math.huge)));
    WelcomeLabel.Size = UDim2.new(1, -8, 0, math.max(20, WelcomeTextHeight + 4));
    MainBoxOuter.Size = UDim2.new(1, 0, 0, 20 + WelcomeLabel.Size.Y.Offset + 4);
    
    local UpdateLogBoxOuter = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Library.OutlineColor;
        Size = UDim2.new(1, 0, 0, 0);
        ZIndex = 154;
    ...
    Library:AddToRegistry(UpdateLogBoxOuter, {
        BackgroundColor3 = 'BackgroundColor';
        BorderColor3 = 'OutlineColor';
    });
    
    local UpdateLogBoxInner = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Color3.new(0, 0, 0);
        BorderMode = Enum.BorderMode.Inset;
        Position = UDim2.new(0, 0, 0, 0);
        Size = UDim2.new(1, 0, 1, 0);
        ZIndex = 155;
        Parent = UpdateLogBoxOuter;
    });
    
    Library:AddToRegistry(UpdateLogBoxInner, {
        BackgroundColor3 = 'BackgroundColor';
    });
    
    local UpdateLogHighlight = Library:Create('Frame', {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Size = UDim2.new(1, 0, 0, 1);
        ZIndex = 156;
        Parent = UpdateLogBoxInner;
    });
    
    Library:AddToRegistry(UpdateLogHighlight, {
        BackgroundColor3 = 'AccentColor';
    });
    
    local UpdateLogSectionLabel = Library:CreateLabel2({
        Size = UDim2.new(1, 0, 0, 18);
        Position = UDim2.new(0, 0, 0, 2);
        Text = "[ " .. UpdateLogTitle .. " ]";
        TextXAlignment = Enum.TextXAlignment.Center;
    ...
    local UpdateLogContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
    ...
    local UpdateLogLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, 2);
        FillDirection = Enum.FillDirection.Vertical;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = UpdateLogContainer;
    });
    ...
    local AccentHex = GetAccentHex();
    for _, Item in next, UpdateLogItems do
        local ItemText = '[<font color="' .. AccentHex .. '">+</font>] ' .. Item;
        ...
        Library:AddToRegistry(ItemLabel, {
            Text = function()
                return '[<font color="' .. GetAccentHex() .. '">+</font>] ' .. Item;
            end;
        });
    end;
    
    wait();
    UpdateLogContainer.Size = UDim2.new(1, -8, 0, UpdateLogLayout.AbsoluteContentSize.Y);
    UpdateLogBoxOuter.Size = UDim2.new(1, 0, 0, 20 + UpdateLogContainer.Size.Y.Offset + 4);
    
    local SettingsBoxOuter = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Library.OutlineColor;
        Size = UDim2.new(1, 0, 0, 0);
        ZIndex = 154;
    ...
    Library:AddToRegistry(SettingsBoxOuter, {
        BackgroundColor3 = 'BackgroundColor';
        BorderColor3 = 'OutlineColor';
    });
    
    local SettingsBoxInner = Library:Create('Frame', {
        BackgroundColor3 = Library.BackgroundColor;
        BorderColor3 = Color3.new(0, 0, 0);
        BorderMode = Enum.BorderMode.Inset;
        Position = UDim2.new(0, 0, 0, 0);
        Size = UDim2.new(1, 0, 1, 0);
        ZIndex = 155;
        Parent = SettingsBoxOuter;
    });
    
    Library:AddToRegistry(SettingsBoxInner, {
        BackgroundColor3 = 'BackgroundColor';
    });
    
    local SettingsHighlight = Library:Create('Frame', {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Size = UDim2.new(1, 0, 0, 1);
        ZIndex = 156;
        Parent = SettingsBoxInner;
    });
    
    Library:AddToRegistry(SettingsHighlight, {
        BackgroundColor3 = 'AccentColor';
    });
    
    local SettingsSectionLabel = Library:CreateLabel2({
        Size = UDim2.new(1, 0, 0, 18);
        Position = UDim2.new(0, 0, 0, 2);
        Text = "[ Settings ]";
        TextXAlignment = Enum.TextXAlignment.Center;
    ...
    local SettingsContentContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
        Size = UDim2.new(1, -8, 0, 0);
        Position = UDim2.new(0, 4, 0, 20);
        ZIndex = 156;
    ...
    local SettingsContentLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, 5);
        FillDirection = Enum.FillDirection.Vertical;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = SettingsContentContainer;
    });
    
    local DontShowAgainContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
        Size = UDim2.new(1, 0, 0, 18);
        ZIndex = 156;
        Parent = SettingsContentContainer;
    ...
    local CheckboxOuter = Library:Create('Frame', {
        ...
        ZIndex = 157;
        Parent = DontShowAgainContainer;
    });
    
    Library:AddToRegistry(CheckboxOuter, {
        BorderColor3 = 'Black';
    });
    ...
    local CheckboxLabel = Library:CreateLabel({
    ...
    local CheckboxRegion = Library:Create('Frame', {
        BackgroundTransparency = 1;
        Size = UDim2.new(1, 0, 1, 0);
        ZIndex = 159;
        Active = true;
    ...
    CheckboxRegion.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
            CheckboxChecked = not CheckboxChecked;
            CheckboxInner.BackgroundColor3 = CheckboxChecked and Library.AccentColor or Library.MainColor;
            CheckboxInner.BorderColor3 = CheckboxChecked and Library.AccentColorDark or Library.OutlineColor;
            
            Library.RegistryMap[CheckboxInner].Properties.BackgroundColor3 = CheckboxChecked and 'AccentColor' or 'MainColor';
            Library.RegistryMap[CheckboxInner].Properties.BorderColor3 = CheckboxChecked and 'AccentColorDark' or 'OutlineColor';
        end;
    end);
    
    if not ShowDontShowAgain then
        DontShowAgainContainer.Visible = false;
    end;
    ...
    wait();
    SettingsContentContainer.Size = UDim2.new(1, -8, 0, SettingsContentLayout.AbsoluteContentSize.Y);
    SettingsBoxOuter.Size = UDim2.new(1, 0, 0, 20 + SettingsContentContainer.Size.Y.Offset + 4);
    
    local function UpdateSizes()
        ContentContainer.CanvasSize = UDim2.new(0, 0, 0, ContentLayout.AbsoluteContentSize.Y + 8);
        local ContentHeight = math.min(ContentLayout.AbsoluteContentSize.Y + 8, 400);
        local TotalHeight = 25 + ContentHeight + 8;
        MainSectionOuter.Size = UDim2.new(1, -16, 0, ContentHeight);
        PopupOuter.Size = UDim2.new(0, Width, 0, TotalHeight);
    end;
    ...
    function Popup:Close()
        if CheckboxChecked and ShowDontShowAgain then
            SavePopupData({
                DontShow = true;
                Hash = CurrentHash;
            });
...
function Library:CreateWindow(...)
    ...
    if type(Config.Title) ~= 'string' then Config.Title = 'Exiled' end
    
    local function GetGreeting()
        local Success, Hour = pcall(function()
            return tonumber(os.date("%H"));
        end);
        if not Success or not Hour then
            return "Welcome";
        end;
    ...
    local function GetFormattedDate()
        ...
        local Success2, DateStr2 = pcall(function()
            return os.date("%m/%d/%Y");
        end);
        return Success2 and DateStr2 or os.date("%x") or "Unknown Date";
    end;
    
    if Config.Title == "Exiled" or Config.Title == "" then
        local Greeting = GetGreeting();
        local Date = GetFormattedDate();
        local AccentHex = Library.AccentColor;
        local R = math.floor(AccentHex.R * 255);
        local G = math.floor(AccentHex.G * 255);
        local B = math.floor(AccentHex.B * 255);
        local HexColor = string.format("#%02X%02X%02X", R, G, B);
        Config.Title = '<font color="' .. HexColor .. '">Exiled</font> - ' .. Greeting .. ' - ' .. Date;
    end;
