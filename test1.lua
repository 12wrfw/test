...
--[[
    Creates a popup notification dialog with update log functionality.
    
    @param Config table - Configuration table with the following optional fields:
        - Title (string): Title for the popup window (default: "Update Log")
        - WelcomeText (string): Welcome message in the Main section (default: "Hello sir, Welcome to Deity Hub! Have a nice day >:3")
        - UpdateLogTitle (string): Title for the update log section (default: "Update Log - [Current Date]")
        - UpdateLogItems (table): Array of strings representing update log items (default: {})
        - ShowDontShowAgain (boolean): Whether to show the "Don't show it again" checkbox (default: true)
        - DontShowAgainKey (string): Key to use for saving "Don't show it again" state (default: "PopupDontShowAgain")
        - OnClose (function): Callback function called when popup is closed
        - Width (number): Width of the popup in pixels (default: 400)
        - Height (number): Height of the popup in pixels (default: 450)
    
    @return Popup object with methods:
        - Popup:Close() - Closes the popup
        - Popup:Destroy() - Destroys the popup
    
    Example usage:
        Library:CreatePopup({
            Title = "Deity Hub Next Generation (Beta)",
            WelcomeText = "Hello sir, Welcome to Deity Hub! Have a nice day >:3",
            UpdateLogTitle = "Update Log - 2/04/2023",
            UpdateLogItems = {
                "Auto Farm Special Boss",
                "Auto Armament Haki V2",
                "Auto Observation Haki V2",
                "Auto Dark King Haki V2",
                "Account Previewer",
                "One Shot Kill (Awake Boss)",
                "Auto Do Weapon"
            },
            ShowDontShowAgain = true,
            DontShowAgainKey = "MyPopupDontShow",
            OnClose = function()
                print("Popup closed!")
            end
        })
...
function Library:CreatePopup(Config)
    ...
    local WelcomeText = Config.WelcomeText or "Hello sir, Welcome to Deity Hub! Have a nice day >:3";
    local UpdateLogTitle = Config.UpdateLogTitle or ("Update Log - " .. os.date("%d/%m/%Y"));
    ...
    -- Check if user has selected "Don't show it again"
    if ShowDontShowAgain and Library.SaveManager then
        local SavedData = Library.SaveManager:GetData();
        if SavedData and SavedData[DontShowAgainKey] == true then
            return nil;
        end;
    end;
    ...
    -- Create main popup frame - matching window style
    ...
    -- Create update log items container
    ...
    -- Create "Alright" button inside Settings section
    local ButtonOuter = Library:Create('Frame', {
        BorderColor3 = Color3.new(0, 0, 0);
    ...
    function Popup:Close()
        if CheckboxChecked and ShowDontShowAgain and Library.SaveManager then
            local SavedData = Library.SaveManager:GetData() or {};
            SavedData[DontShowAgainKey] = true;
